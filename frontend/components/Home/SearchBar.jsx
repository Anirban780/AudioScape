 ...
 * 4. Keyboard Navigation: ArrowUp/Down, Enter, Escape shortcuts.
...
const SearchBar = ({ onSelectTrack }) => {
  ...
  /**
   * Executes HTTP GET request to backend search proxy.
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto px-2 sm:px-4">
      ...
      >
        <Search
          size={20}
          className={`cursor-pointer ${theme === "dark" ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-black"}`}
          onClick={() => {
            if (query.trim()) {
              saveSafeRecentSearch(query);
              fetchSearchResults(query, "", true);
        ...
        <Input
          ref={inputRef}
          type="text"
          placeholder="Search songs (Press Enter for YouTube API)..."
          value={query}
      ...
      {isFocused && (
        <div
          ...
          className={`absolute left-0 w-full mt-2 p-2 rounded-md shadow-lg max-h-80 overflow-y-auto transition-opacity duration-300 z-50 ${
            theme === "dark" ? "bg-gray-900 text-white border border-gray-800" : "bg-white text-black border border-gray-200"
          }`}
        >
          {/* Enhanced Recent Searches Section */}
          {(!query.trim() || (results.length === 0 && !loading)) && recentSearches.length > 0 && (
            <div className="p-2 mb-1 border-b border-gray-800/40">
              <div className="flex items-center justify-between mb-2 px-1">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Clock size={13} className="text-indigo-400" /> Recent Searches
                </p>
                <button
                  type="button"
                  onMouseDown={clearAllRecentSearches}
                  className="text-xs font-medium text-gray-400 hover:text-red-400 transition-colors flex items-center gap-1"
                >
              ...
              <div className="flex flex-wrap gap-1.5">
                {recentSearches.map((term, i) => (
                  <div
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full cursor-pointer transition-all duration-200 ${
                      theme === "dark"
                        ? "bg-gray-800 hover:bg-indigo-950/60 text-gray-200 border border-gray-700/60 hover:border-indigo-500/50"
                        : "bg-gray-100 hover:bg-indigo-50 text-gray-800 border border-gray-200 hover:border-indigo-300"
                    }`}
                  >
                    ...
                    <button
                      ...
                      className="p-0.5 rounded-full hover:bg-gray-700/50 text-gray-400 hover:text-red-400 transition-colors"
                    >
                      <X size={12} />
                    </button>
                  </div>
          ...
          {query.trim() && searchStatusMsg && (
            <div
              className={`px-3 py-1.5 mb-2 rounded text-xs flex items-center justify-between font-medium ${
            ...
            >
              ...
              {searchSource !== "youtube_api" && (
                <button
                  ...
                  className="text-[10px] text-indigo-300 hover:text-white font-semibold underline cursor-pointer ml-2 flex-shrink-0"
                >
                  Press Enter ↵
                </button>
              )}
              ...
              return (
                <div
                  ...
                  className={`flex items-center p-2 cursor-pointer rounded transition-colors ${
                    isSelected
                      ? theme === "dark"
                        ? "bg-gray-700 text-white font-medium border-l-4 border-indigo-500"
                        : "bg-gray-300 text-black font-medium border-l-4 border-indigo-600"
                ...
                >
                  <img
                    ...
                    alt="Thumbnail"
                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-md mr-3 flex-shrink-0 object-cover"
                  />
          ...
          {query.trim() && !loading && results.length === 0 && (
            <div className="p-4 text-center">
              <p className="text-sm text-gray-400 mb-2">No local database tracks found for "{query}"</p>
              <button
                type="button"
                onMouseDown={() => fetchSearchResults(query, "", true)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md bg-indigo-600 hover:bg-indigo-500 text-white transition-colors shadow-md"
              >
          ...
          {/* Loading Indicator */}
          {loading && <p className="p-2 text-center text-xs font-medium text-gray-400">Loading search results...</p>}
