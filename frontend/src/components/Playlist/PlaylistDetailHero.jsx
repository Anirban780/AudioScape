...
/**
 * ============================================================================
 * PLAYLIST DETAIL HERO (PlaylistDetailHero.jsx)
 * ============================================================================
 *
 * WHAT THIS FILE DOES:
 * Renders the top hero section for a specific playlist (/playlists/:id).
 * Features:
 * - Large 2x2 mosaic or custom cover image
 * - Dynamic blurred background based on cover art
 * - Playlist title, description, track count, and total duration
 * - Play All and Shuffle CTA buttons
 * - Inline edit trigger for renaming the playlist
 *
 * WHY IT WAS DESIGNED THIS WAY:
 * 1. Cinematic feel: Uses a blurred copy of the mosaic art as the background,
 *    layered with dark gradients, to make the page feel premium and deep.
 * 2. Information hierarchy: Title is massive, secondary stats are small and crisp.
 * 3. Clear CTAs: "Play All" stands out in brand purple.
 */
const PlaylistDetailHero = ({
    ...
    // Filter out failed thumbnails for the mosaic
    ...
    /**
     * Renders either the custom cover, a 2x2 mosaic, a single thumbnail, or a gradient stub.
     */
    const renderCoverArt = (isBackground = false) => {
        // If it's for the background, we just want a single image to blur
        if (isBackground) {
        ...
        // Custom Cover URL takes precedence
        if (coverUrl) {
        ...
        <div className="relative w-full overflow-hidden rounded-[32px] border border-[var(--color-border-strong)] shadow-2xl mb-8 bg-[var(--color-surface-raised)] min-h-[320px] flex items-end">
            ...
            <div className="relative z-10 w-full flex flex-col md:flex-row items-end md:items-center gap-6 md:gap-10 p-6 sm:p-10">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-end w-full">
                    ...
                    <h1 
                        className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-white tracking-tighter leading-none mb-3 sm:mb-4 line-clamp-2 cursor-pointer hover:text-[var(--color-primary)] transition-colors"
                        onClick={onEdit}
                    ...
                    {/* Stats Row */}
                    <div className="flex items-center gap-4 text-xs sm:text-sm font-semibold text-slate-300 mb-6 flex-wrap">
                        <span className="flex items-center gap-1.5">
                            <span className="text-white">{trackCount}</span> {trackCount === 1 ? "song" : "songs"}
                        </span>
